...
class Admin extends MY_Controller {
	...
	//delete PostOffice
	function deletepostoffice()
	{
		$this->validate_admin();
		if ($this->uri->segment(3) != '' && is_numeric($this->uri->segment(3))) {
			$where['id'] = $this->uri->segment(3);
			$this->base_model->delete_record(
			$this->db->dbprefix('markers'), 
			$where
			);
			...
			elseif($this->uri->segment(4) != '' && $this->uri->segment(4) == 'moderator')
				...
				redirect('admin/viewAllPostOffices');
		}
	...
	//view post office
	function viewpostoffice()
	{
		$this->validate_admin();
		$this->data['content'] 	= 'admin/postoffices/view_only_postoffices';
			$this->data['pagetitle'] 	= 'View PostOffice';
			$this->_render_page('templete/admin/layout', $this->data);
			/*
		...
		if ($this->uri->segment(3) && is_numeric($this->uri->segment(3))) {
			...
			$table, 
			$condition,
			$select 				= 'Id, officename, Pincode, Type, DeliveryStatus, Divisionname, Regionname, Circlename, Taluk, Districtname, Statename, Telephone, Related_Suboffice, Related_Headoffice, Lng, Lat', 
			$order_by = '' 
			);
			$this->data['details'] 	= $records;
			$this->data['content'] 	= 'admin/postoffices/view_only_postoffices';
	...
	function settings()
	{
		...
		$this->data['pagetitle'] 			= 'Update Settings';
	...
	//View All PostOffices
	function viewAllPostOffices()
	{
		
		$this->validate_admin();

		$allPostoffices	= $this->base_model->run_query(
		"SELECT m.* FROM markers m"
		);
		$this->data['allPostoffices'] 	= $allPostoffices;
		$this->data['pagetitle'] 		= 'All PostOffices';
		$this->data['active_menu'] 	= 'users';
		$this->data['content'] 		= 'admin/postoffices/view_all_postoffices';
		$this->_render_page('templete/admin/layout', $this->data);
	}
	//add post office
	function addpostoffice()
	{
		...
		$this->form_validation->set_rules('officename', 'office Name', 'trim|required');
		
		if ($this->input->post()) {
			if ($this->form_validation->run() == true) {
				$inputdata['id'] = $this->input->post('id');
				$inputdata['officename'] = $this->input->post('officename');
				$inputdata['address'] 	= $this->input->post('address');
				$inputdata['pincode'] 	= $this->input->post('pincode');
				$inputdata['type'] 	= $this->input->post('type');
				$inputdata['deliverystatus'] 	= $this->input->post('deliverystatus');
				$inputdata['divisionname'] 	= $this->input->post('divisionname');
				$inputdata['regionname'] 	= $this->input->post('regionname');
				$inputdata['circlename'] 	= $this->input->post('circlename');
				$inputdata['Taluk'] 	= $this->input->post('Taluk');
				$inputdata['Districtname'] 	= $this->input->post('Districtname');
				$inputdata['statename'] 	= $this->input->post('statename');
				$inputdata['Telephone'] 	= $this->input->post('Telephone');
				$inputdata['Related_Suboffice'] 	= $this->input->post('Related_Suboffice');
				$inputdata['Related_Headoffice'] 	= $this->input->post('Related_Headoffice');
				...
				$this->prepare_flashmessage($msg, 0);
				redirect('admin/viewAllPostOffices', 'refresh');
			}
			else {
				$this->prepare_flashmessage(validation_errors(), 1);
				redirect('admin/addpostoffice', 'refresh');
			}
		}
		if ($this->uri->segment(3) != '' && is_numeric($this->uri->segment(3))) {
			$this->data['data'] 	= $this->base_model->run_query(
			"select * from ".$this->db->dbprefix('markers')
			." where id=".$this->uri->segment(3)
			);
			$this->data['id'] 		= $this->uri->segment(3);
			$this->data['pagetitle'] 	= 'Update Postoffice';
		}
		else {
			$this->data['data'] 	= array();
			$this->data['id'] 		= '';
			$this->data['pagetitle'] 	= 'Add postoffice';
		}
		
		$this->data['active_menu'] 	= '';
		$this->data['content'] 		= 'admin/gs';
		$this->_render_page('templete/admin/layout', $this->data);
	...
	function editpostoffice($id = '')
	{
		...
		$this->data['title'] = "Edit PostOffice";
